...
REMEDIATION = {
    ...
    "Cookieのセキュリティ属性（Secure）": "発行するCookieに Secure 属性を付与し、HTTPS通信でのみ送信されるようにする。",
    "Cookieのセキュリティ属性（HttpOnly）": "JavaScriptから読み取る必要のないCookieには HttpOnly 属性を付与し、XSS発生時の窃取を防ぐ。",
    "Cookieのセキュリティ属性（SameSite）": "Cookieに SameSite=Lax（または Strict）を設定し、CSRF攻撃のリスクを下げる。",
    "既知の重大脆弱性": "該当ソフトウェアを直ちに修正済みバージョンへ更新する。緊急パッチが提供されている場合は先行適用も検討する。",
    "サポート終了(EOL)ソフトウェアの可能性": "サポート中のバージョン系列へ計画的に移行する。移行までの間は追加の防御策（WAF等）を検討する。",
    "MTA-STS": "_mta-sts.<ドメイン> にTXTレコードを公開し、https://mta-sts.<ドメイン>/.well-known/mta-sts.txt に mode: enforce のポリシーを設置する。メール受信側の配送経路を暗号化強制にする対策。",
